...
def handle_text(event):
    ...
    if event.message.text.startswith("送信完了"):
        ...
        line_bot_api.reply_message(
            event.reply_token, [TextSendMessage(text="受け付けました♪")]
        )
        return
    ...
    if event.message.text in available_texts:
        r.set(
            ...
            str(available_texts.index(event.message.text) + 1).zfill(2),
        )
        # Rich menu
        richmenu_list = [
            "richmenu-4318d8c8dba62fc14de3fced2943e413",
        ...
        line_bot_api.link_rich_menu_to_user(
            event.source.user_id,
            richmenu_list[available_texts.index(event.message.text)],
        )
        ...
        json = {
            ...
            "hero": {
                ...
                + "/imgs/mercari_%s.png" % (r.get(event.source.user_id)),
                "size": "full",
                "aspectRatio": "1:1",
                "aspectMode": "cover",
            },
        ...
        reply_text = (
            "載せたいテキストを変更したい時には↓をタップしてね。今は「"
            + available_texts[int(r.get(event.source.user_id)) - 1]
            + "」に設定されてるよ♪"
        )
...
def handle_image(event):
    ...
    image_base.paste(
        image_overlay, (0, image_base.height - image_overlay.height), image_overlay
    )
...
def handle_follow(event):
    ...
    line_bot_api.link_rich_menu_to_user(
        event.source.user_id, "richmenu-4318d8c8dba62fc14de3fced2943e413"
    )
